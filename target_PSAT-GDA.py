...
def data_load(args): 
    ...
    dsets["test_aug_fix"] = ImageList_idx_fix(txt_test, transform=image_test())
    dset_loaders["test_aug_fix"] = DataLoader(dsets["test_aug_fix"], batch_size=train_bs*3, shuffle=False, num_workers=args.worker, drop_last=False)
...
def train_source(args):
    ...
    acc_init = 0
    max_iter = args.max_epoch_src * len(dset_loaders["source_tr"])
...
def train_target(args):
    ...
    max_iter = args.max_epoch_tgt * len(dset_loaders["target"])
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 and args.cls_par > 0:
            ...
            netB.eval()
            netC.eval()
            pred_near, opts_aug, feas_FC = obtain_label(dset_loaders['test_aug'], netF, netB, netC, args)
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            netC.eval()
            if args.dset=='VISDA-C':
                acct, acc_list_t = cal_acc(dset_loaders['test'], netF, netB, netC, True)
                accs, acc_list_s = cal_acc(dset_loaders['source_te'], netF, netB, netC, True)
                acc_all = [accs, acct]
                log_str = 'Task: {}, Iter:{}/{}; Accuracy on source = {:.2f}%. Accuracy on target = {:.2f}%'.format(args.name, iter_num, max_iter, accs, acct) + '\n' + acc_list_s +'\n' + acc_list_t
            else:
                acct, _ = cal_acc(dset_loaders['test'], netF, netB, netC, False)
                accs, _ = cal_acc(dset_loaders['source_te'], netF, netB, netC, False)
                acc_all = [accs, acct]
                log_str = 'Task: {}, Iter:{}/{}; Accuracy on source = {:.2f}%. Accuracy on target = {:.2f}%'.format(args.name, iter_num, max_iter, accs, acct)
    ...
    if args.issave:   
        ...
        np.save(osp.join(args.output_dir, "acc_all" + ".npy"), acc_all_)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_epoch_src', type=int, default=10, help="max iterations")
    ...
    parser.add_argument('--output', type=str, default='./ckpskdr0/target_gskd_tem/')
    parser.add_argument('--output_src', type=str, default='./ckpskdr0/source_tt/')
