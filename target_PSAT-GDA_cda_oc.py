...
def data_load_oc(args): 
    ## prepare data
    dsets = {}
    dset_loaders = {}
    train_bs = 64

    path_amazon = "/home/ts/projects/SYJ/ACM_contiual_sfda/data_shot/office/amazon_list.txt"
    txt_tar_amazon = open(path_amazon).readlines()
    path_dslr = "/home/ts/projects/SYJ/ACM_contiual_sfda/data_shot/office/dslr_list.txt"
    txt_tar_dslr = open(path_dslr).readlines()
    path_webcam = "/home/ts/projects/SYJ/ACM_contiual_sfda/data_shot/office/webcam_list.txt"
    txt_tar_webcam = open(path_webcam).readlines()

    tr_txt_a = torch.load(osp.join(args.data_sp_src, "tr_txt_a.pt")) 
    te_txt_a = torch.load(osp.join(args.data_sp_src, "te_txt_a.pt"))
    tr_txt_d = torch.load(osp.join(args.data_sp_src, "tr_txt_d.pt")) 
    te_txt_d = torch.load(osp.join(args.data_sp_src, "te_txt_d.pt"))
    tr_txt_w = torch.load(osp.join(args.data_sp_src, "tr_txt_w.pt")) 
    te_txt_w = torch.load(osp.join(args.data_sp_src, "te_txt_w.pt")) 

    dsets["amazon_te"] = ImageList(te_txt_a, transform=image_test())
    dset_loaders["amazon_te"] = DataLoader(dsets["amazon_te"], batch_size=train_bs, shuffle=False, num_workers=args.worker, drop_last=False)
    dsets["amazon_tr"] = ImageList(tr_txt_a, transform=image_train())
    dset_loaders["amazon_tr"] = DataLoader(dsets["amazon_tr"], batch_size=train_bs, shuffle=True, num_workers=args.worker, drop_last=False)

    dsets["dslr_te"] = ImageList(te_txt_d, transform=image_test())
    dset_loaders["dslr_te"] = DataLoader(dsets["dslr_te"], batch_size=train_bs, shuffle=False, num_workers=args.worker, drop_last=False)
    dsets["dslr_tr"] = ImageList(tr_txt_d, transform=image_train())
    dset_loaders["dslr_tr"] = DataLoader(dsets["dslr_tr"], batch_size=train_bs, shuffle=True, num_workers=args.worker, drop_last=False)

    dsets["webcam_te"] = ImageList(te_txt_w, transform=image_test())
    dset_loaders["webcam_te"] = DataLoader(dsets["webcam_te"], batch_size=train_bs, shuffle=False, num_workers=args.worker, drop_last=False)
    dsets["webcam_tr"] = ImageList(tr_txt_w, transform=image_train())
    dset_loaders["webcam_tr"] = DataLoader(dsets["webcam_tr"], batch_size=train_bs, shuffle=True, num_workers=args.worker, drop_last=False)

    dsets["amazon_test"] = ImageList(txt_tar_amazon, transform=image_test())
    dset_loaders["amazon_test"] = DataLoader(dsets["amazon_test"], batch_size=train_bs*3, shuffle=False, num_workers=args.worker, drop_last=False)
    dsets["amazon_target"] = ImageList_idx(txt_tar_amazon, transform=image_train())
    dset_loaders["amazon_target"] = DataLoader(dsets["amazon_target"], batch_size=train_bs, shuffle=True, num_workers=args.worker, drop_last=False)
    dsets["amazon_test_aug"] = ImageList_idx_aug(txt_tar_amazon, transform=image_test())
    dset_loaders["amazon_test_aug"] = DataLoader(dsets["amazon_test_aug"], batch_size=train_bs*3, shuffle=False, num_workers=args.worker, drop_last=False)
    
    dsets["dslr_test"] = ImageList(txt_tar_dslr, transform=image_test())
    dset_loaders["dslr_test"] = DataLoader(dsets["dslr_test"], batch_size=train_bs*3, shuffle=False, num_workers=args.worker, drop_last=False)
    dsets["dslr_target"] = ImageList_idx(txt_tar_dslr, transform=image_train())
    dset_loaders["dslr_target"] = DataLoader(dsets["dslr_target"], batch_size=train_bs, shuffle=True, num_workers=args.worker, drop_last=False)
    dsets["dslr_test_aug"] = ImageList_idx_aug(txt_tar_dslr, transform=image_test())
    dset_loaders["dslr_test_aug"] = DataLoader(dsets["dslr_test_aug"], batch_size=train_bs*3, shuffle=False, num_workers=args.worker, drop_last=False)

    dsets["webcam_test"] = ImageList(txt_tar_webcam, transform=image_test())
    dset_loaders["webcam_test"] = DataLoader(dsets["webcam_test"], batch_size=train_bs*3, shuffle=False, num_workers=args.worker, drop_last=False)
    dsets["webcam_target"] = ImageList_idx(txt_tar_webcam, transform=image_train())
    dset_loaders["webcam_target"] = DataLoader(dsets["webcam_target"], batch_size=train_bs, shuffle=True, num_workers=args.worker, drop_last=False)
    dsets["webcam_test_aug"] = ImageList_idx_aug(txt_tar_webcam, transform=image_test())
    dset_loaders["webcam_test_aug"] = DataLoader(dsets["webcam_test_aug"], batch_size=train_bs*3, shuffle=False, num_workers=args.worker, drop_last=False)

    return dset_loaders


def prepare_data_oc(args): 
    ## prepare data
    path_amazon = '/home/ts/projects/SYJ/ACM_contiual_sfda/data_shot/office/amazon_list.txt' 
    path_dslr = '/home/ts/projects/SYJ/ACM_contiual_sfda/data_shot/office/dslr_list.txt'
    path_webcam = '/home/ts/projects/SYJ/ACM_contiual_sfda/data_shot/office/webcam_list.txt'
    
    txt_src_a = open(path_amazon).readlines()
    txt_src_d = open(path_dslr).readlines()
    txt_src_w = open(path_webcam).readlines()
    ...
    dsize_d = len(txt_src_d)
    tr_size_d = int(0.9*dsize_d)
    print(dsize_d, tr_size_d, dsize_d - tr_size_d)
    tr_txt_d, te_txt_d = torch.utils.data.random_split(txt_src_d, [tr_size_d, dsize_d - tr_size_d])
    torch.save(tr_txt_d, osp.join(args.data_sp_src, 'tr_txt_d.pt'))
    torch.save(te_txt_d, osp.join(args.data_sp_src, 'te_txt_d.pt'))

    dsize_w = len(txt_src_w)
    tr_size_w = int(0.9*dsize_w)
    print(dsize_w, tr_size_w, dsize_w - tr_size_w)
    tr_txt_w, te_txt_w = torch.utils.data.random_split(txt_src_w, [tr_size_w, dsize_w - tr_size_w])
    torch.save(tr_txt_w, osp.join(args.data_sp_src, 'tr_txt_w.pt'))
    torch.save(te_txt_w, osp.join(args.data_sp_src, 'te_txt_w.pt'))

    print("data office has splited")
    return 0
...
def test_target(netF, netB, netC, args, ):
    netF.eval()
    netB.eval()
    netC.eval()
...
def test_source(netF, netB, netC, args):
    dset_loaders = data_load(args)
    netF.eval()
    netB.eval()
    netC.eval()
    if args.dset=='VISDA-C':
        acc, acc_list = cal_acc(dset_loaders['source_te'], netF, netB, netC, True)
        log_str = '\nTraining: {}, Task: {}, Accuracy = {:.2f}%'.format(args.name_net, args.name, acc) + '\n' + acc_list
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_data', type=str, default='./data_sp/')
    ...
    prepare_data_oc(args)

    dset_loaders = data_load_oc(args)
